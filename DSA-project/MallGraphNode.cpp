...
#include <iostream>
using namespace std;
MallGraphNode::MallGraphNode() :id(-1), mallID(""), x(0.0), y(0.0), edgeHead(NULL) {}
MallGraphNode::MallGraphNode(int i, string mid, float xCoord, float yCoord) :id(i), mallID(mid), x(xCoord), y(yCoord), edgeHead(NULL) {}
MallGraphNode::~MallGraphNode() {
    GraphEdge* curr = edgeHead;
    while (curr) {
        GraphEdge* next = curr->getNext();
        delete curr;
        curr = next;
    }
}
...
void MallGraphNode::addEdge(int to, int weight) { GraphEdge* newEdge = new GraphEdge(to, weight); newEdge->setNext(edgeHead); edgeHead = newEdge; }